...
-- SKILL DRILL: Create three more inquities to search for employees who were borh in 1953,'54,'55
SELECT first_name, last_name
FROM employees
...
-- Shorten the previous table to a nickname (e.g., retirement_infor becomes "ri")
SELECT ri.emp_no,
    ri.first_name,
ri.last_name,
    de.to_date
...
-- Joining departments and dept_manager tables (shortened)
SELECT d.dept_name,
     dm.emp_no,
     dm.from_date,
     dm.to_date
...
-- Employee count by department number, adding ORDER BY to organize everyting by department #
SELECT COUNT(ce.emp_no), de.dept_no
FROM current_emp as ce
LEFT JOIN dept_emp as de
ON ce.emp_no = de.emp_no
...
-- SKILL DRILL: Sales and Development teams information
SELECT ri.emp_no,
	ri.first_name,
	ri.last_name,
	di.dept_name
